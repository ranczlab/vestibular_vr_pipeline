...
# - **Comparison exports:** per-metric statistics, plot data, and figures saved to `condition_comparison/` within the experiment day.
#
# %%
...
if GENERATE_CONDITION_COMPARISON:
    ...
    if len(condition_metrics_frames) < 2:
        ...
        if long_df.empty:
            ...
            preferred_order = ["Apply_halt_day3", "Apply_halt_day4", "No_halt"]
            ...
            if len(available_conditions) < 2:
                ...
                metric_condition_map = {
                    "peak": ["Apply_halt_day3", "Apply_halt_day4", "No_halt"],
                    "mean_fluorescence_2_to_8s": ["Apply_halt_day3", "Apply_halt_day4", "No_halt"],
                    "onset_time": ["Apply_halt_day3", "Apply_halt_day4"],
                    "decay_tau": ["Apply_halt_day3", "Apply_halt_day4"],
                }
                ...
                comparison_output_dir = Path(main_data_dir) / "condition_comparison"
                comparison_output_dir.mkdir(parents=True, exist_ok=True)

                outputs_written = False

                for metric_name in COMPARISON_METRICS:
                    target_conditions = metric_condition_map.get(metric_name, available_conditions)
                    condition_subset = [
                        cond
                        for cond in preferred_order
                        if cond in target_conditions and cond in long_df["condition"].unique()
                    ]
...
def _compute_grand_average_for_condition(
    ...
    cfg = CONDITION_COMPARISON_CONFIG.get(condition_key)
    if cfg is None:
...
if 'condition_metrics_frames' not in locals() or not condition_metrics_frames:
    ...
    overlay_output_dir = Path(main_data_dir) / 'condition_comparison'
    ...
    if not overlay_plan:
        ...
        for condition_key in overlay_plan:
            ...
            condition_label_for_plot = CONDITION_COMPARISON_CONFIG[condition_key]["label"]
